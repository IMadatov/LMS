...
namespace LearnSystem.Controllers
{
        ...
        ) : BaseController
    {
        ...
        public async Task<ActionResult<UserDto?>> Me()
        {
            ...
            return await FromServiceResult(userBaseCrudService.GetByIdAsync(Guid.Parse(userId), UserProfile,async context =>
            {
                var query = context.Queryable;
                query = query.Include(u => u.StatusUser);
                return query;
            }));
        }
        ...
        public async Task<ActionResult<LanguageDto>> GetUserLanguage()
            => await FromServiceResultBaseAsync(userService.UserLanguage(UserProfile));


        [HttpPost]
        public async Task<ActionResult<LanguageDto>> ChangeLanguage(string lang) =>
            await FromServiceResultBaseAsync(userService.ChangeLanguage(lang, UserProfile));
