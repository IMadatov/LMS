...
public abstract partial class BaseCrudService<TEntity, TDto, TDtoFull, TKey, TUserKey>
{
        ...
        IQueryable<TEntity> query)
    {
        ...
        var interfaceType = typeof(IGlobalFilterExpression<,>).MakeGenericType(typeof(TEntity), typeof(TKey));

        Type? globalFilter = typeof(TEntity).Assembly
            .GetTypeAssignableFromInterface(interfaceType);
    ...
    {
        var interfaceType = typeof(ISelectExpression<,,>).MakeGenericType(typeof(TEntity), typeof(TDto), typeof(TKey));
